...
import { Model, Provider, models, providers, removeProvider } from "../state/ModelProvidersContext";
...
const DeprecatedDefunctOrPreview = (props: { model: Model }) => (
    <>
        ...
        <Show when={props.model.meta?.productionReady === false && props.model.available}>
                    ...
                    <Accordion collapsible class="w-full" defaultValue={["item-1"]}>
                        <AccordionItem value="item-1" class="border-none">
                            <div class="flex flex-row items-center text-sm font-bold">
                                ...
                                <Button variant={"ghost"} class="w-7 h-7 p-0" size={"sm"} onClick={() => removeProvider(provider.id)}>
                                    <Trash2 class="h-4 w-4" />
                                    <span class="sr-only">Toggle</span>
                                </Button>
                                <AccordionTrigger
                                    class="w-7 h-7 p-2 rounded-lg flex items-center justify-center hover:bg-accent hover:text-accent-foreground active:bg-accent/60 active:scale-95"
                                />
                            </div>
                            <AccordionContent>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                ...
                                                <TableHead class="h-8"></TableHead>
                                                <TableHead class="h-8 text-right">Context Window</TableHead>
                                                <TableHead class="h-8 text-right">Output Tokens</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                                ...
                                                {model => (
                                                    <TableRow>
                                                        <TableCell class="pr-2">
                                                            {topModels.includes(model.name) ? <Sparkles class="w-4 h-4 text-blue-400" /> : ""}
                                                        </TableCell>
                                                        ...
                                                        <TableCell class="text-right">{model.meta?.contextWindow || "-"}</TableCell>
                                                        <TableCell class="text-right">{model.meta?.maxOutputTokens || "-"}</TableCell>
                                                    </TableRow>
                                                )}
                                            </For>
                                        </TableBody>
