...
const SidebarHeader = (props: SidebarHeaderProps) => {
  return (
    <div class="flex flex-col w-full bg-gray-50 p-4 space-y-4 items-center justify-between relative border-b-[1px] border-b-gray-200">
      ...
      <div class="w-full flex relative">
        ...
        />
        <div
          class="absolute right-0 top-0 h-full flex items-center justify-center p-2"
          style={
    ...
    >
      <div class="flex-1 flex flex-col p-4 items-start gap-2">
        <div class="flex flex-row w-full items-center gap-2">
          <span class="flex-1 font-bold whitespace-break-spaces truncate line-clamp-1 text-left">
            {props.title}
          </span>
        ...
        <span
          class="text-left font-medium line-clamp-2 h-[2.25rem]"
      ...
      <div
        class="flex-0 flex flex-col item-center justify-center h-full"
        style={{
...
const MessagesSidebar = (props: { isOpen: boolean }) => {
  ...
  return (
    <div
      class={`transition-all duration-300 overflow-x-hidden ${
        props.isOpen ? "w-[300px]" : "w-0"
      }`}
    >
      <div class="flex flex-col w-full bg-sidebar-background h-full border-r border-border">
        ...
        <div class="flex-1 overflow-y-auto overflow-x-hidden">
          ...
          <Show when={query().length > 0}>
            <SearchResultsList query={query()} setQuery={setQuery} />
          </Show>
        </div>
      </div>
    </div>
