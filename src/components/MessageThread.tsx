...
const AttachmentGroup = (props: { attachments: AttachmentGroup }) => {
  return (
    <div class="w-full flex flex-row items-center justify-end space-x-1">
      ...
      <For each={props.attachments.messages}>
        {(m) => (
          <Tooltip>
            ...
            <TooltipTrigger>
              <FileText class="w-7 h-7 hover:text-primary/80 hover:rotate-6 hover:scale-110 transition-all">
                {m.id}
              </FileText>
            </TooltipTrigger>
          </Tooltip>
...
const MessageThread = (props: MessageThreadProps) => {
  const [useAsChallengeTreeId, setUseAsChallengeTreeId] = createSignal<
    string | null
  >(null);
      ...
      <For each={messageGroups()}>
        {(message) =>
          message.type === "MESSAGE" ? (
            <IndividualMessage
              onUseAsChallenge={() =>
                setUseAsChallengeTreeId(message.message.id)
              }
              id={message.message.id}
              message={message.message.content}
              role={message.message.role}
              onRegenerate={props.onRegenerate}
              onEdit={() => props.onEdit(message.message.id)}
            />
          ) : (
