...
const providerNames = [CUSTOM_PROVIDER_DEFAULT, ...knownProviders.map(p => p.name).sort()];
...
const AddProviderDialog = (props: AddProviderFormProps) => {
    const [selectedProvider, setSelectedProvider] = createSignal<string>(CUSTOM_PROVIDER_DEFAULT);
    ...
    const onAddProvider = async () => {
        ...
        const providerId = await addProvider({
            ...
            apiKey: apiKey(),
            endpoint: isCustomProvider() ? endpoint() : getEndpointByProvider(selectedProvider()) || '',
            ...(p?.icon ? { icon: p.icon } : {})
        });
    ...
    return (
        <div class="fixed inset-0 z-50 overflow-y-auto bg-background/60" style={{
            "backdrop-filter": "blur(2px)",
            opacity: props.show ? 1 : 0,
            transition: "opacity 0.2s",
            "pointer-events": props.show ? "auto" : "none"
        }}>
            <form class="min-w-[36rem] max-w-[42rem] mx-auto mt-12" style={{
                ...
                <div class="flex flex-col w-full mx-auto p-6 space-y-2 bg-white rounded-lg shadow-lg">
                    <div class="flex flex-row items-center justify-between">
                        ...
                        <Button class="flex flex-row spaxe-x-2" variant={"ghost"} size={"icon"} onClick={props.onHide}>
                            <X />
                        </Button>
                    </div>
                    ...
                    <Select
                        ...
                        options={providerNames.filter(p => providers().every(pr => pr.name !== p))}
                        itemComponent={props =>
