...
const [selectionData, setSelectionData] = createSignal<SelectionData | null>(
  null
);
...
const accordionTriggerClasses =
  "text-lg font-bold h-7 hover:no-underline hover:text-slate-900";
...
const MessageWithEditableVariableNames = (
  props: MessageWithEditableVariableNamesProps
) => {
  ...
  const onFinishedEditing = () => {
    ...
    props.renameVariable(
      wrapNameAsVar(isEditing()),
      wrapNameAsVar(newName().trim())
    );
    setIsEditing("");
  };

  const onKeyUp: JSX.EventHandlerUnion<
    HTMLInputElement,
    KeyboardEvent,
  ...
  return (
    <div
      ref={$div}
      class={
        "p-4 rounded-lg bg-slate-100 whitespace-pre-wrap " +
        (props.left ? "mr-2" : "ml-2")
      }
    >
      <For each={parts}>
        {(part, i) =>
          part.length === 0 ? (
            ...
            <span
              class="__mammal_variable__"
              onClick={() => onStartEditing(part)}
            >
              {isEditing() === part ? (
                <TextField
                  class="px-1 bg-white"
...
const ChallengeDialog = (props: ChallengeDialogProps) => {
  ...
  const [metricCaseSensitivity, setMetricCaseSensitivity] =
    createSignal<boolean>(false);
  const [metricType, setMetricType] = createSignal<
    "Exact" | "Contains" | "Regex Match"
  >("Contains");
  ...
  const [showVariablePopup, setShowVariablePopup] =
    createSignal<boolean>(false);
  ...
  const handleSelection = (index: number) => () => {
    ...
    // @ts-ignore
    const { top: dialogTop, left: dialogLeft } =
      $dialog?.getBoundingClientRect() || { left: 0, top: 0 };
  ...
  createEffect(() => {
    ...
    const newTitle =
      rootNodes().find((n) => benchmarkIdRoot === n.path.split(".")[0])
        ?.title || "";
    setBenchmarkTitle(newTitle);
  });

  createEffect(() => {
    const existingVariables = messages()
      .map((m) => m.content.match(variableRegex))
      .filter((v) => !!v)
      .flat();
    setVariables(
      existingVariables.map((name) => ({
        name,
        value: "",
      }))
    );
            ...
            <div class="flex flex-row items-center bg-background rounded-md border px-4 py-2">
              <Accordion collapsible class="w-full">
                <AccordionItem value="item-1" class="border-none">
                  <AccordionTrigger class={accordionTriggerClasses}>
                  ...
                  <AccordionContent class="mt-2">
                    ...
                    <Show
                      when={selectedText().length > 0 && showVariablePopup()}
                    >
                      <div
                        class="absolute bg-white border shadlow-lg p-4 rounded z-50"
                        style={{
                      ...
                      >
                        ...
                        <div class="flex justify-end space-x-2">
                          ...
                          <Button onClick={handleCreateVariablePopup}>
                            Create
                          </Button>
                        </div>
                      </div>
                    ...
                    <div class="flex flex-col space-y-1">
                      <div class="flex flex-row items-center justify-end">
                        ...
                        <Switch
                          ...
                          checked={metricCaseSensitivity()}
                          onChange={setMetricCaseSensitivity}
                        >
                          <SwitchLabel>Case Sensitive:</SwitchLabel>
                          <SwitchControl>
                            <SwitchThumb />
                          </SwitchControl>
                        </Switch>
