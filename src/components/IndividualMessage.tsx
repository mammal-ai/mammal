...
const tryDelete = (treeId: string) => async () => {
    const sure = await window.confirm("Are you sure you want to delete this message and all subsequent replies?")
    if (!sure) {
        return
    }
    ...
    if (parentId === null) {
        console.error("Couldn't reset active message because parent could not be found", parentId)
        return
    }
    const parentNode = await getMessage(parentId)
    if (parentNode === null) {
        console.error("Couldn't reset active message because parent node could not be found", parentId)
        return
    }
...
const MessageButtons = (props: MessageButtonsProps) =>
    <div class={'absolute px-2 z-10 bottom-[-0.5rem] min-w-full flex flex-row justify-end ' + (props.align === "LEFT" ? "left-0" : "right-0")}>
        <div class='h-[1.8rem] flex flex-row items-center justify-end space-x-2 bg-background px-[1rem] text-sm text-gray-400 rounded-full shadow-sm' style={{
...
const DirectionButtons = (props: DirectionButtonsProps) => {
    ...
    const position = () => siblingCount() > 1 ? `${index() + 1}/${siblingCount() + 1}` : "1"
    ...
    const goToSibling = (forward: boolean) => () => {
        ...
        const currentIndex = currentSiblings.findIndex(n => n.path === props.treeId)
        if (currentIndex === -1) {
...
const getRole = (role: string) => role in roles ? roles[role as keyof typeof roles] : roles.unknown;
...
const IndividualMessage = (props: IndividualMessageProps) => {
    ...
    function debounce<T>(fn: Function, ms: number) {
        return (...args: unknown[]) => new Promise<T>((resolve) => {
            if (debounceTrigger) {
