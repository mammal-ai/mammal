...
const readCsv = (file: File, options: { header: boolean }) =>
  new Promise<Papa.ParseResult<unknown>>((resolve) => {
    Papa.parse(file, {
      ...
      <DialogContent class="max-h-[90vh] min-w-[700px] overflow-hidden flex flex-col">
        ...
        <div class="flex-1 flex flex-col space-y-4 overflow-hidden">
            ...
            <div class="mt-2 p-4 overflow-y-auto bg-gray-100 flex flex-col space-y-2">
              <For each={models()}>
                {(m) => (
                  <label
                    class={
                      show(m.name.toLowerCase(), modelFilter())
                        ? "bg-gray-100"
                        : "hidden"
                    }
                  >
          ...
          <Show when={data().length === 0 && variables().length > 0}>
            <div>
              <div class="mb-1">
                <span>Select a source csv file to supply variables:</span>
                <span>
                  <For each={stillRequiredVars()}>
                    {(v) => (
                      <>
                        {" "}
                        <span class="px-2 py-0.5 rounded-sm bg-slate-200 font-bold">
                          {v}
                        </span>
                      </>
                    )}
          ...
          <Show when={data().length > 0 && stillRequiredVars().length > 0}>
            <div>
              <span class="mb-1 font-bold text-red-700">
          ...
          <div class="flex flex-row justify-end space-x-2">
            ...
            <Button variant={"default"} onClick={runBenchmark}>
              Run Benchmark
            </Button>
          </div>
        </div>
