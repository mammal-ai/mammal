...
const refreshRootNodes = async () => {
  ...
  const titles = await db.select<{ id: string; title: string }>(
  ...
  const rn = nodes.map((n) => ({
    title:
      titles.find((t) => t.id === n.path.split(".")[0])?.title ||
      "Unknown Title",
    path: n.path,
    firstMessage: n.data,
  }));
  ...
  const rootIdsWithoutTitles = rootIds.filter(
    (id) => !titles.find((t) => t.id === id)
  );
  const generatedTitles = (
    await Promise.all(
      rootIdsWithoutTitles.map(async (id) => {
        const messages = await db.select<{ path: string; message: string }>(
          "SELECT path, message FROM message_view WHERE path LIKE $1",
          [`${id}.%`]
        );
        ...
        const dequotedTitle = /^".*"$/.test(trimmedTitle)
          ? trimmedTitle.substring(1, trimmedTitle.length - 2)
          : trimmedTitle;
        if (dequotedTitle) {
          await db.execute(
            "INSERT INTO thread_titles (id, title) VALUES ($1, $2)",
            [id, dequotedTitle]
          );
        }
  ...
  setRootNodes(
    nodes.map((n) => ({
      title:
        allTitles.find((t) => t.id === n.path.split(".")[0])?.title ||
        "Unknown Title",
      path: n.path,
      firstMessage: n.data,
    }))
...
const updateThreadTitle = async (threadId: string, title: string) => {
  await db.execute("UPDATE thread_titles SET title = $1 WHERE id = $2", [
    title,
    threadId,
  ]);
  refreshRootNodes();
...
const cascadeDelete = async (treeId: string) => {
  ...
  const descendantCount = await db.select<{ count: number }>(
    "SELECT COUNT(path) count FROM messages WHERE path LIKE $1",
    [`${rootPath}.%`]
  );
...
const setActiveThread = async (treeId: string) => {
  ...
  const findLatestNode = (
    node: MPTreeNodeWithChildren<MessageData>
  ): MPTreeNodeWithChildren<MessageData> => {
